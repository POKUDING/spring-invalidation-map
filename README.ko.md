...
```text
Spring 메타데이터 ─┬─ RequestMappingHandlerMapping ─► 엔드포인트
...
| `expand-read-associations` | `true` | 읽기 집합을 `@OneToMany`/`@ManyToOne`/`@OneToOne`/`@ManyToMany`/`@Embedded`/`@ElementCollection` 연관을 따라 넓힙니다 |
...
연관 확장은 한 단계에서 멈추지 않고 전이적으로 닫습니다. `ClassInfo → ClassInfoLabel → Label`
처럼 손자 엔티티가 응답에 실리는 사슬에서 한 단계만 보면 `Label` 을 쓰는 엔드포인트 뒤에 오래된
값이 화면에 남기 때문입니다. 닫힘 비용은 실측했습니다 — 엔티티 57개인 프로젝트에서 한
엔드포인트가 잡는 최대치는 4개(7%)로, 한 단계 확장(최대 3개)과 사실상 같았습니다. 연관이 더
촘촘한 도메인에서 이 값이 커질 수 있으므로 스타터가 분석 후 최대 집합 크기를 로그에 남기고,
전체 엔티티의 절반을 넘으면 `warn` 으로 올립니다.

쓰기 집합은 **cascade 가 걸린 연관만** 따라 넓힙니다. `cascade = ALL`/`PERSIST`/`MERGE`/
`REMOVE` 나 `orphanRemoval = true` 가 붙은 연관은 부모를 저장할 때 DB 가 자식 행을 실제로
쓰기 때문이고, cascade 가 없는 연관은 부모를 저장해도 자식이 쓰이지 않으므로 넣지 않습니다.
이 확장은 매핑에 적힌 사실을 옮기는 것이라 `expand-read-associations` 로 끄지 않습니다 —
끄면 실제로 쓰이는 엔티티가 `writes` 에서 빠집니다.

## 계약 경계

이 라이브러리는 `base-packages` 안에서, 핸들러 메서드로부터 JVM 이 읽을 수 있는
바이트코드로 도달하는 코드의 엔티티 접근을 보고합니다. 그 경계 밖은 아래와 같습니다.
...
```bash
./gradlew check                 # 코어·스타터를 Boot 4.x 로, boot3Test 를 Boot 3.3.5 로
./gradlew publishToMavenLocal   # 키가 설정되어 있으면 서명합니다
```

`boot3Test` 는 같은 스타터 테스트를 Spring Boot 3.3.5 클래스패스로 다시 돌립니다. 하나의
아티팩트가 두 세대를 지원한다는 주장을 단정이 아니라 테스트로 확인합니다.
